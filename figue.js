...
var figue = function () {
	...
	function getDeterminedVectors(k, vectors){
		var v = new Array();
		var t = new Array();
		var i = new Array();
		v = vectors;
		...
		var n = 0.0;
		while(n < k){
			var index = Math.floor(n*vectors.length/k);
			t.push(v[index]);
			i.push(index);
			n++;
		}
		return {'vectors':t,'indices':i};
	}
	...
	function kmeans (k, vectors) {
		...
		var t = getDeterminedVectors(k, vectors) ;
		if (t == null)
			return null ;
		else
			centroids = t.vectors ;
		...
		return { 'centroids': centroids , 'assignments': assignments, 'clusterSizes': clusterSizes };
